...
export class HybridRetriever implements Retriever {
  ...
  private readonly subRetrievers: Retriever[];
  ...
  async retrieve(query: RetrievalQuery, chunks: ContextChunk[]): Promise<RetrievalResult[]> {
    ...
    const perRetriever = await Promise.all(
      this.subRetrievers.map((retriever) => retriever.retrieve({ ...query, topK: wideTopK, minScore: 0 }, chunks)),
    );

    const fused = new Map<string, { chunk: ContextChunk; rrf: number; scoreBreakdown: Record<string, number>; reasons: string[] }>();
    ...
    perRetriever.forEach((results, retrieverIndex) => {
      ...
      results.forEach((result, rank) => {
        ...
        if (result.reasons?.length) entry.reasons.push(...result.reasons);
...
export function hybridRetriever(options: HybridRetrieverOptions = {}): HybridRetriever {

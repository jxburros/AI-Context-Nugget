...
export function selectSourcesByPolicy(sources, taskType, rules) {
    const rule = rules.find((candidate) => candidate.taskType === taskType);
    if (!rule)
        return { selected: [], missingRequired: [], coverageWarning: `No context rule was defined for task type "${taskType}".` };
    ...
    for (const kind of rule.requiredKinds ?? []) {
        ...
        if (matches.length === 0)
            missingRequired.push(`kind:${kind}`);
    ...
    for (const kind of rule.optionalKinds ?? [])
        selected.push(...sources.filter((source) => source.kind === kind));
...
export function rankSourcesByQuery(sources, query, budget = {}) {
